...
all_scenes = ['ignatius','caterpillar','truck']
# all_scenes = ['truck']
...
for scene in all_scenes:
    if scene == 'truck':
        ...
        obj = '66'
    # ply_file = f"output/{scene}_2D/object_{obj}/train/ours_30000/fuse_post.ply"
    ply_file = f"../SuGaR/output/refined_mesh/{scene}/fuse_post.obj"
    # ply_file = f"output/{scene}_2D/train/ours_30000/fuse_post.ply"
    string = f"OMP_NUM_THREADS=4 python {script_dir}/eval_tnt/run.py " + \
        f"--dataset-dir data/{scene} " + \
        f"--traj-path data/{scene}/{scene}_COLMAP_SfM.log " + \
        f"--ply-path {ply_file}"
    print(string)
    os.system(string)
