...
def loss_cls_3d_cosine(features, predictions, k=5, lambda_val=2.0, max_points=200000, sample_size=800):
    """
    Compute the neighborhood consistency loss for a 3D point cloud using Top-k neighbors
    and cosine similarity, normalized to [0, 1].
    ...
    similarity = F.cosine_similarity(sample_preds.unsqueeze(1), neighbor_preds, dim=-1)

    # Fetch neighbor predictions using indexing
    

    # Compute cosine similarity loss
    loss = similarity.sum(dim=-1).mean()
    # Normalize loss into [0, 1]
    num_classes = predictions.size(1)
    normalized_loss = 1 - (loss + 1) / 2  # Normalize to [0, 1]
